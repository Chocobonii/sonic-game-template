...
class LL_World_Generator{
    public:
        ...
        int LL_Chunk_Get_Height(int _world_X, int _world_Y);   // function to get the height of the world
        // -------------------------------------------------------
};
// -------------------------------------------------------
// **********************************************
...
void LL_World_Generator::LL_Chunk_Create(int p, int zx, int zy){
 ...
 for(int z = 0; z < 16; z++){
    for(int x = 0; x < 16; x++){
        ...
        // ------------------------------
        // - MAKE THE TREES IN THE ROOM -
        // ------------------------------ç
        int LL_Noise_random = Noise(0,(float)(std::abs(zx*15+x)), (float)(std::abs(zy*15+z)))*world_freq;
        if(LL_Noise_random > 40 && LL_Noise_random < 43 && height_map1 > 50){
            LL_New_Chunk->tree_inmap[x][z] = 1;
        }
    }
 }
 // --------------------------------------------------------------------------------------------------------
        ...
        glPushMatrix();
            for(int z = 0; z < 15; z++){
                for(int x = 0; x < 15; x++){
                    ...
                    glPopMatrix();
                    if((int)LL_Auxiliar_P->tree_inmap[x][z] == 1){
                        glPushMatrix();
                            glTranslatef(x+(LL_Auxiliar_P->LL_Zone_x*15),LL_Auxiliar_P->height_map[x][z],z+(LL_Auxiliar_P->LL_Zone_y*15));
                            glDisable(GL_CULL_FACE);
                            LL_Models->LL_Model_Draw(6);
                            LL_Models->LL_Model_Draw(7);
...
// --------------------------------------------------------
int LL_World_Generator::LL_Chunk_Get_Height(int _world_X, int _world_Y){
   return 40+ValueNoise_2D((float)(std::abs(_world_X)), (float)(std::abs(_world_Y)))*world_freq;
}
// --------------------------------------------------------
